...
var Read = async function (req, res) {
  try {
    ...
    futil.logger.debug(
      "\n" +
        futil.shtm() +
        "- [ REQ PARAMS  createdby] | INFO " +
        util.inspect(req.headers.createdby)
    );
...
var ReadTaskByStatus = async function (req, res) {
  try {
    ...
    futil.logger.debug(
      "\n" +
        futil.shtm() +
        "- [ REQ PARAMS  page] | INFO " +
        util.inspect(req.headers.page)
    );
    futil.logger.debug(
      "\n" +
        futil.shtm() +
        "- [ REQ PARAMS  rows] | INFO " +
        util.inspect(req.headers.rows)
    );
    futil.logger.debug(
      "\n" +
        futil.shtm() +
        "- [ REQ PARAMS  offset] | INFO " +
        util.inspect(req.headers.offset)
    );
        ...
        if (days > 0) {
          ...
          if (task_status != "Overdue") {
            if (!user_lat && !user_lon && !vehicle_lat && !vehicle_lon) {
              try {
                var res_update = await Task.update(
                  { task_status: "Overdue" },
                  {
                    where: {
                      id: id,
